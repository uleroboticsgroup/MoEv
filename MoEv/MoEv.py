		...
		if conf["testSaveModels"]["enabled"]:
			...
			for key in sorted(var.keys()):
				if conf["Models"][key]["enabled"] and (key != "Save_Models"):
					...
					#We check if model imported has arguments in the function to import it
					if "arguments" in conf["Models"][key].keys():
						if conf["Models"][key]["arguments"]["import"]: 
							import_argument = conf["Models"][key]["arguments"]["import"]
							function_argument = conf["Models"][key]["arguments"]["function"]

							module_argument = importlib.import_module(import_argument)
							argument_function = getattr(module_argument, function_argument)

							model = function(argument_function())
							model.modelName = key
						else:
							argument = conf["Models"][key]["arguments"]["argument"]
					...
					if conf["Models"][key]["GridSearch"]["enabled"]:
						...
						model = test.gridSearch(model, self.get_X_train(), self.get_y_train(), conf["Models"][key]["GridSearch"]["dictionary"],model_type)
						model.modelName = key
